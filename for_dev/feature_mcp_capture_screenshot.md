...
**How (tech).**
- **Window activation**: before enumerating, the tool calls `unminimize()` + `show()` + `set_focus()` on the Tauri `WebviewWindow` (cheap no-ops if the window is already visible/frontmost). Then `tokio::time::sleep(Duration::from_millis(300))` to let SkyLight register the window. Without this, the first call after launch always failed with "window not found" because xcap's `kCGWindowListOptionOnScreenOnly` filter excludes windows that haven't been activated on the window server yet.
- **Window finding**: same logic as the in-app snapshot — `pid == our_pid || app_name().to_lowercase().contains("fenceymd")`. The `is_minimized()` filter excludes windows confirmed minimized (an `is_minimized()` error does NOT exclude — better to attempt than to spuriously report "window not found"). The captured image is downscaled so its longest edge is ≤1600px before PNG-encoding.
...
**Gotchas.**
- Headless launches (`open /Applications/Foo.app` from a non-GUI shell) don't create a window, so the tool returns "FenceyMD window not found." This is a verification-side issue, not a code bug. (For the deeper headless-support story see `plan_mcp_screenshot_v2.md` — headless-Chrome re-render, planned for a future version.)
- **Occluded / background windows capture blank.** On macOS, capturing a window whose WebView is fully occluded or suspended (e.g. the app is behind another window, or was just launched into the background) can yield an all-white image even though the tool *found* the window and returned valid PNG dimensions. The capture is reliable when the FenceyMD window is visible/frontmost. Verified live: a backgrounded dev instance returned a blank 1600×1193 PNG; the same build frontmost captures real content. (Confirmed during the v1.1 MCP review.)
- **First call after launch used to return "window not found"** even on a GUI-launched app. xcap's `Window::all()` uses `kCGWindowListOptionOnScreenOnly`, which excludes windows that haven't been "activated" on SkyLight's active list. The fix: the tool now calls `unminimize()` + `show()` + `set_focus()` on the Tauri window, then `tokio::time::sleep(300ms)` before enumerating. The 300ms is the async window-server activation delay; the first call after launch used to always error, now it works. Total tool latency is ~500ms (300ms settle + ~200ms capture/encode). Verified live: 10 consecutive `capture_screenshot` calls right after a fresh `open /Applications/FenceyMD.app` all returned valid PNGs with no external `osascript activate` needed.
- macOS sometimes rejects large images at the clipboard — but we're not using the clipboard here, so no issue.
- Payload size is bounded by the ≤1600px-longest-edge downscale: a typical capture is tens-to-low-hundreds of KB of base64 (e.g. a 1600×1193 reader view ≈ 40 KB PNG → ~53 KB base64), well within MCP-response limits — no longer the multi-MB concern of a raw Retina grab.
